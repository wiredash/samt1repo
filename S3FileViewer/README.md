...
This is a sample template for S3FileViewer - Below is a brief explanation of what we have generated for you:
...
```bash
sam logs -n HelloWorldFunction --stack-name s3fileviewer --tail
```
...
```bash
aws cloudformation delete-stack --stack-name s3fileviewer
```
